...
#define BMP_EMPTY(bitmap, index) ((bitmap[index/8] & (0x01<<(7-index%8))) == 0)
#define BMP_SET(bitmap, index) 	(bitmap[i/8] |= (((UINT8)0x01)<<(7-i%8))) 
#define BMP_FULL(bitmap, index) ((bitmap[index/8] & (0x01<<(7-index%8))) != 0)
...
static void rdafwr_q_push(const UINT8 cmd, const UINT32 version, const UINT32 return_msg, const UINT32 fid)
{
	...
	if(full==1)
	{
		uart_print("Error: rdafwrq full");
		while(1);	//Stop it. Error
...
//빈 index를 찾아라.
	...
	if(empty==1)
	{
		...
		//if(tp_v==0)
		uart_print("Error: rdafwr not found");
		while(1);
...
//		uart_printf("%d %d %d %d", mac[i], mac[i+1], mac[i+2], mac[i+3]);
	uart_printf("DS_write_wr) version %d", version);
