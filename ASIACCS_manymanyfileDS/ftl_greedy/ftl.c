...
#define DS_MAX_BLOCKS (1024)
#define DS_INODE_BLKS_PER_BANK (DS_MAX_BLOCKS / NUM_BANKS) //1024/8=128개의 block할당
...
#define DS_dec_metablk_vpn(bank, DS_metablk_lbn) (g_misc_meta[bank].DS_cur_metablk_vpn[DS_metablk_lbn]--)
...
//  uart_printf("block full!");
  //uart_printf("*******************************block full!***********************************");
        ...
        //mem_copy(FTL_BUF(bank)+ sizeof(UINT32) * PAGES_PER_BLK + sizeof(SGX_LBA) * PAGES_PER_BLK, g_misc_meta[bank].is_sgx_lpn, \
        sizeof(UINT8) * PAGES_PER_BLK);
...
/*     uart_printf("garbage_collection bank %d, vblock %d",bank, vt_vblock); */
    ...
    //mem_copy(g_misc_meta[bank].is_sgx_lpn, FTL_BUF(bank)+ sizeof(UINT32) * PAGES_PER_BLK + sizeof(SGX_LBA) * PAGES_PER_BLK,  \
    sizeof(UINT8) * PAGES_PER_BLK);
...
static void format(void)
{
 ...
 // #define MAPBLKS_PER_BANK    (((PAGE_MAP_BYTES / NUM_BANKS) + BYTES_PER_PAGE - 1) / BYTES_PER_PAGE)    //RAM을 통해 각 뱅크에 맵핑 페이지 수
   ...
   // uart_printf("DS_NHASH_ADDR : %x %x\n", DS_NHASH_ADDR, DS_NHASH_BYTES);
    ...
    mem_set_dram(DS_FILETB_ADDR, NULL, DS_FILETB_BYTES);
    mem_set_dram(DS_FILELOC_ADDR, -1, DS_FILELOC_BYTES);
...
void DS_store_inode(INODE* inode)  //ino가 block주소를 상징한다 생각하면 된다.
{
    ...
    UINT32 bank = (inode->no>>4) % 8;
    UINT32 DS_metablk_lbn = (inode->no>>4) / 8;
   ...
   //FTL chunk는 128bytes의 각 FTL 덩어리를 의미함.
    ...
    UINT8 new_loc;
    UINT32 copy_bytes=0;
    INODE* v_inode = (INODE*)DS_PA_to_VA(inode);
    //inode의 시작 vpn을 저장한다.
...
if(1){
    ...
    for(ftl_chunk=0; ftl_chunk<num_FTLchunk; ftl_chunk++)
    {
        ...
        //여긴 호출되어서는 안되는 부위임. -> manyfile은 오직 작은 파일만 가정한다. ftl이 커지면 안된다.
        /*
        if(copy_bytes % BYTES_PER_PAGE == 0 )
        {
            ...
            if(inode->no & 0x0f == 0x0f)
                DS_inc_metablk_vpn(bank, DS_metablk_lbn);
            DS_metablk_vpn += 8*(inode->no & 0x0f);
    ...
    //남은 쩌리짱도 처리하기.
    if(copy_bytes > 0 && (YAME_RD!=1))  //완벽한 코드가 아님 주의
//    if(copy_bytes > 0)
    {
        //DS_inc_metablk_vpn(bank, DS_metablk_lbn);
        DS_metablk_vpn = DS_get_metablk_vpn(bank, DS_metablk_lbn);
        DS_metablk_vpn += 8*(inode->no & 0x0f); //한 아이노드가 연속된 8페이지를 가진다
        new_loc = read_dram_8(DS_FILELOC_ADDR + inode->no);
        // out-of-place이므로 증가해서 저장해야한다.
        new_loc++;
/*
        if(inode->no == 1)  uart_print("s inode 1");
        else if(inode->no == 2)  uart_print("s inode 2");
        else if(inode->no == 3)  uart_print("s inode 3");
        else if(inode->no == 4)  uart_print("s inode 4");
        else if(inode->no == 5)  uart_print("s inode 5");
        else if(inode->no == 6)  uart_print("s inode 6");
        else if(inode->no == 7)  uart_print("s inode 7");
        else if(inode->no == 8)  uart_print("s inode 8");
        else if(inode->no == 9)  uart_print("s inode 9");
        else if(inode->no == 10)  uart_print("s inode 10");
        else if(inode->no == 11)  uart_print("s inode 11");
        else if(inode->no == 12)  uart_print("s inode 12");
        else if(inode->no == 13)  uart_print("s inode 13");
        else if(inode->no == 14)  uart_print("s inode 14");
        else if(inode->no == 15)  uart_print("s inode 15");
        else if(inode->no == 16)  uart_print("s inode 16");
        else uart_print("s inode out");
*/
/*
        if(new_loc==-1) uart_print("store new_loc=-1");
        if(new_loc>2)   uart_print("store new_loc>2");
        if(new_loc==0)  uart_print("store new_loc=0");
        if(new_loc==1)  uart_print("store new_loc=1");
        if(inode->no<0 || inode->no>4096)   uart_print("load inode > 4096");
*/
        write_dram_8(DS_FILELOC_ADDR + inode->no, new_loc);
        DS_metablk_vpn += new_loc;  //각 inode마다 더할 값이 다를 수 있음을 명심하자.
        //uart_printf("store %d %d", inode->no, new_loc);
    ...
    //여기는 제거한다.DS_set_metablk_vpn은 무조건 쓰지 않을 예정. 
    /*
    DS_set_metablk_vpn(bank, DS_metablk_lbn, vpn);  //꼭 필요한가?
    //마지막에 다시 다음 쓰레드가 로드할 것을 대비해서 이렇게 줄이는거다.
    if((inode->no & 0x0f) != 0x0f && manyfiles_ld==1)
       DS_dec_metablk_vpn(bank, DS_metablk_lbn);
       */
}
...
void DS_load_inode(INODE* inode, UINT32 ino)
{
    ...
    vpn += 8*(ino & 0x0f);  
    vpn += read_dram_8(DS_FILELOC_ADDR + ino);
/*
            if(ino == 1)  uart_print("l inode 1");
        else if(ino == 2)  uart_print("l inode 2");
        else if(ino == 3)  uart_print("l inode 3");
        else if(ino == 4)  uart_print("l inode 4");
        else if(ino == 5)  uart_print("l inode 5");
        else if(ino == 6)  uart_print("l inode 6");
        else if(ino == 7)  uart_print("l inode 7");
        else if(ino == 8)  uart_print("l inode 8");
        else if(ino == 9)  uart_print("l inode 9");
        else if(ino == 10)  uart_print("l inode 10");
        else if(ino == 11)  uart_print("l inode 11");
        else if(ino == 12)  uart_print("l inode 12");
        else if(ino == 13)  uart_print("l inode 13");
        else if(ino == 14)  uart_print("l inode 14");
       else  if(ino == 15)  uart_print("l inode 15");
        else if(ino == 16)  uart_print("l inode 16");
        else uart_print("l inode out");
*/
/*
    if(read_dram_8(DS_FILELOC_ADDR + ino) == -1) uart_print("load new_loc=-1");
    if(read_dram_8(DS_FILELOC_ADDR + ino) > 2) uart_print("load new_loc>2");
    if(read_dram_8(DS_FILELOC_ADDR + ino) == 0) uart_print("load new_loc=0");
    if(read_dram_8(DS_FILELOC_ADDR + ino) == 1) uart_print("load new_loc=1");
    if(ino<0 || ino>4096)   uart_print("load inode > 4096");
  */
    //uart_printf("load %d %d", ino, read_dram_8(DS_FILELOC_ADDR + ino));
  ...
  //  if(ino & 0x0f == 0x0f)
    //여기도 제거한다. DS_inc_metablk_vpn은 무조건 쓰지 않을 계획.
    //DS_inc_metablk_vpn(bank, DS_metablk_lbn);   //여기가 이해가 안된다. 이건 추후에 store할떄를 대비한거임. 한자리 밑에 스토어하기 위해서 증가시킨거임.
   ...
   // uart_printf("num_FTLchunk : %d", num_FTLchunk);
    ...
    for(chunk_idx=0; chunk_idx<num_FTLchunk; chunk_idx++)
    {
        //uart_printf("?");
        //page단위로 읽어와야한다.
        //manyfile은 작은 파일이므로 여긴 호출되선 안된다.
        /*
        if(load_bytes % BYTES_PER_PAGE == 0 )
        {
            uart_printf("l not here!!");
