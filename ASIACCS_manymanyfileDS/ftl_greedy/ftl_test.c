...
*/
    ...
    char name[NAME_LEN] = "0001_00000.txt";
    int i, j;
    //short int fids1[300];
    //static short int fids2[4096-300];    //오버플로우 문제
    short int fid;
    ...
    /*
    테스트코드가 매우 불안정하다. 아무래도 600bytes함수할당한게 SDRAM 바운더리초과 에러를 내는 것 같다. 
    */
 ...
 for(i=0; i<4095; i++)
    ...
    */
    ...
    for(i=0; i<4095; i++)
    {
        ...
        if(i<300)   fids1[i] = fid;
        else        fids2[i-300] = fid;

        uart_printf("create: %s %d",name,fid);
        //DS_file_close(fid);
    ...
    for(i=0; i<4095; i++)
    {
        ...
        if(i<300)   fid = fids1[i];
        else        fid = fids2[i-300];

        DS_file_close(fid);
        uart_printf("close %s %d",name,fid);
   ...
   // uart_print("write");
    for(i=0; i<4095; i++)
    {
     ...
     //       uart_printf("test..r.%d ",i);
       ...
       // DS_file_close(fid);
        uart_printf("write: %s %d",name,fid);
    }
