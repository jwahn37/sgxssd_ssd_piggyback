...
static int DS_get_inode_no()
{
    if(ds_superblock.num_inode == 4096)
        uart_print("ILOVEU4096");
...
static void DS_nhash_insert(const char name[NAME_LEN], const UINT32 ino, INODE* inode)
{
   ...
   // UINT32 table_addr = (DS_VA_to_PA(&ds_superblock.name_hash[nhash_func(name)]));
    ...
    else
    {
        ...
        {
            //uart_printf("NONONONOONONO!!!COLLISION!!!!!!!!!!!!!!!!!!!!!!!!!");
            if(cur->ino == ino)
           // if(memcmp(cur->inode->name, name) == 0) 
            {
                ;//
                //uart_printf("ERR[DS_nhash_insert]");
              //  uart_print("Error [DS_nhash_insert] : Duplicated Name");
                //return NULL;
                return;
            }
            cur = cur->next;
...
void DS_inode_set_size(UINT32 fid, UINT32 offset, UINT32 size)
{
   ...
   // uart_printf("DS_inode_set_size: fid(%d), ino(%d)", fid, inode->no)
...
//
//
static UINT32 DS_filetb_read(int fid)
{
    return read_dram_32(DS_FILETB_ADDR + fid*sizeof(UINT32));
}
static void DS_filetb_write(int fid, INODE* inode)
{
    write_dram_32(DS_FILETB_ADDR + fid*sizeof(UINT32), (UINT32)inode);
}
...
static UINT32 DS_filetb_insert(INODE* inode)
{
   ...
   //해당 노드가 비어있는지 체크. 빌떄까지 뻉뻉이.
    ...
    //if((INODE*)DS_filetb_read(next_fid)==NULL){uart_print("fdtableinsertNULL");}
    while((INODE*)DS_filetb_read(next_fid)!=NULL)
    {
        //uart_printf("?");
        //next_fid = (next_fid + 1) % DS_MAX_FILE;
        next_fid ++;
        if(next_fid == DS_MAX_FILE) next_fid = 1;
        i++;
        if(i>DS_MAX_FILE)
        {
  ...
  //  ds_superblock.file_table[next_fid] = inode;
    DS_filetb_write(next_fid, inode);
...
static INODE* DS_filetb_remove(const UINT32 fid)
{
    ...
    INODE* inode = (INODE*)DS_filetb_read(fid);
    //ds_superblock.file_table[fid]=NULL;
    DS_filetb_write(fid, NULL);
    return inode;
}
...
static INODE* DS_filetb_get_inode(const UINT32 fid)
{
    ...
    return (INODE*)DS_filetb_read(fid);
}
...
/*
    if(inode->no == 1)  uart_print("c inode=1");
    else if(inode->no == 2)  uart_print("c inode=2");
    else if(inode->no == 3)  uart_print("c inode=3");
    else if(inode->no == 4)  uart_print("c inode=4");
    else if(inode->no == 5)  uart_print("c inode=5");
    else if(inode->no == 6)  uart_print("c inode=6");
    else if(inode->no == 7)  uart_print("c inode=7");
    else if(inode->no == 8)  uart_print("c inode=8");
    else if(inode->no == 9)  uart_print("c inode=9");
    else if(inode->no == 10)  uart_print("c inode=10");
    else if(inode->no == 11)  uart_print("c inode=11");
    else if(inode->no == 12)  uart_print("c inode=12");
    else if(inode->no == 13)  uart_print("c inode=13");
    else if(inode->no == 14)  uart_print("c inode=14");
    else if(inode->no == 15)  uart_print("c inode=15");

    if(fid==1)  uart_print("c fid=1");
    else if(fid==2)  uart_print("c fid=2");
    else if(fid==3)  uart_print("c fid=3");
    else if(fid==4)  uart_print("c fid=4");
    else if(fid==5)  uart_print("c fid=5");
    else if(fid==6)  uart_print("c fid=6");
    else if(fid==7)  uart_print("c fid=7");
    else if(fid==8)  uart_print("c fid=8");
    else if(fid==9)  uart_print("c fid=9");
    else if(fid==10)  uart_print("c fid=10");
    else if(fid==11)  uart_print("c fid=11");
    else if(fid==12)  uart_print("c fid=12");
    else if(fid==13)  uart_print("c fid=13");
    else if(fid==14)  uart_print("c fid=14");
    else if(fid==15)  uart_print("c fid=15");
    else uart_print("c fid err"); 
    */
 //   if(fid!=1)  uart_printf("IHATEU3000");
    return fid;
/*
    // /////2. Insert name hash
...
UINT32 DS_file_open(const char name[NAME_LEN])
{
   ...
   // key=DS_inode_get_key(inode);
    ...
    //5. Insert file table
    fid =  DS_filetb_insert(inode);
   // uart_printf("open ino : name:%s inode: %d fid: %d", name, ino, fid);
...
UINT8 DS_file_close(UINT32 fid)
{
    ...
    /*
     if(fid==1)  uart_print("cl fid=1");
    else if(fid==2)  uart_print("cl fid=2");
    else if(fid==3)  uart_print("cl fid=3");
    else if(fid==4)  uart_print("cl fid=4");
    else if(fid==5)  uart_print("cl fid=5");
    else if(fid==6)  uart_print("cl fid=6");
    else if(fid==7)  uart_print("cl fid=7");
    else if(fid==8)  uart_print("cl fid=8");
    else if(fid==9)  uart_print("cl fid=9");
    else if(fid==10)  uart_print("cl fid=10");
    else if(fid==11)  uart_print("cl fid=11");
    else if(fid==12)  uart_print("cl fid=12");
    else if(fid==13)  uart_print("cl fid=13");
    else if(fid==14)  uart_print("cl fid=14");
    else if(fid==15)  uart_print("cl fid=15");
    else uart_print("cl fid err");
    */
    //uart_print("3");l
    INODE* inode = DS_filetb_remove(fid);
    /*
     if(inode->no == 1)  uart_print("cl inode=1");
    else if(inode->no == 2)  uart_print("cl inode=2");
    else if(inode->no == 3)  uart_print("cl inode=3");
    else if(inode->no == 4)  uart_print("cl inode=4");
    else if(inode->no == 5)  uart_print("cl inode=5");
    else if(inode->no == 6)  uart_print("cl inode=6");
    else if(inode->no == 7)  uart_print("cl inode=7");
    else if(inode->no == 8)  uart_print("cl inode=8");
    else if(inode->no == 9)  uart_print("cl inode=9");
    else if(inode->no == 10)  uart_print("cl inode=10");
    else if(inode->no == 11)  uart_print("cl inode=11");
    else if(inode->no == 12)  uart_print("cl inode=12");
    else if(inode->no == 13)  uart_print("cl inode=13");
    else if(inode->no == 14)  uart_print("cl inode=14");
    else if(inode->no == 15)  uart_print("cl inode=15");
    */
  // uart_printf("fi_%d_%d", fid, inode->no);
// uart_printf("3 %d", inode->no);
    //2. store inode
    //if(YAME_RD==0)    //read일때는 굳이 store할필요가 없다.
...
UINT32 DS_get_version_increased(const UINT32 fid)
{
   ...
   // return ds_superblock.file_table[fid]->version;
    ...
    ((INODE*)DS_filetb_read(fid)) -> version++;
    return ((INODE*)DS_filetb_read(fid)) -> version;
