...
static UINT8 DS_auth_create(UINT8* name, UINT8* key, UINT8* mac);
...
/*						
					...
					else	//read after write : reserved 영역인 fid 에 version 정보가 있다.
					{
						...
						if(rdafwr_q_pop(cmd, version, &return_msg, &fid))
						{
							...
							else if(cmd==DS_CLOSE_RD)
							{
								version = version;
								/* code */
								//uart_printf("v(%d), rt(%d)", version, return_msg);
							//	uart_printf("fid(%d), key(%d)", fid, *(UINT8*)DS_get_filekey(fid));
							//	uart_printf("fid(%d) key(%d) ret(%d)", fid, ((UINT8*)DS_get_filekey(fid))[0], return_msg) ;
								DS_rdafwr_make_mac(version, return_msg, mac, -1, DS_get_tempkey_closerd());
								DS_remove_tempkey_closerd();
							}
							...
							{	
								...
								if(fid!=FILE_NOT_EXISTS)	
									DS_rdafwr_make_mac(version, return_msg, mac, file_size, (UINT8*)DS_get_filekey(fid));
							}
							else if(cmd==DS_CREATE_RD)
							{
								DS_rdafwr_make_mac(version, return_msg, mac, -1, (UINT8*)DS_get_filekey(fid));
							}
							else if(cmd==DS_WRITE_RD)
							{
								//uart_printf("fid(%d)", fid);
				...
				{
				//	num_ipfs_rd++;
					uart_print("Read from Regular zone");
			...
			else
			{
				...
				{
					//num_ipfs_wr++;
					uart_print("Write to Regular zone");
...
void DS_open_wr(SGX_PARAM sgx_param)
{
	//UINT8 *data_buf;
	UINT32 fid;
	char name[NAME_LEN]={0,};//초기화이즈베리임포턴트 -> 인증
	UINT32 version;
	UINT32 size;
...
//	fid=1;
	...
	*/
	...
	if(fid==FILE_NOT_EXISTS || DS_auth_open(version, name, mac, (UINT8*)DS_get_filekey(fid)))
	{
	...
	{	
		uart_print("Open verificaion fail.");
		//인증실패시 닫고 오류메세지.
		DS_file_close(fid);
		rdafwr_q_push(DS_convert_RD_cmd(sgx_param.cmd), version, -1, fid);	//얘는 후에 READ 할때 인증됩니다.
...
void DS_create_wr(SGX_PARAM sgx_param)
{
	...
	int i;
	UINT32 fid;
	char name[NAME_LEN]={0,};	//초기화이즈베리임포턴트
	UINT32 version;
	UINT8 mac[MAC_SIZE];
	UINT8 key[KEY_SIZE];
	...
	//HMAC_DELAY(3);
	//if(1)
	if(DS_auth_create(name, key, mac))
	{
		//create fail이면 -1이 리턴
...
//		uart_printf("DS_create_wr %d", fid);
	...
	//	rdafwr_q_push(DS_convert_RD_cmd(sgx_param.cmd), version, fid);
		...
		uart_print("Create Auth Fail");
		rdafwr_q_push(DS_convert_RD_cmd(sgx_param.cmd), version, -1, fid);
...
void DS_close_wr(SGX_PARAM sgx_param)
{
	...
	//HMAC_DELAY(3);
	//if(1)
	if(DS_auth_close(version, sgx_param.fid, (UINT8*)DS_get_filekey(sgx_param.fid), mac))
	{
		//close 
	...
	//	uart_printf("DS_close_wr fid:%d rtmsg : %d", sgx_param.fid, rtmsg);
		...
		uart_printf("Close Auth Fail");
		rdafwr_q_push(DS_convert_RD_cmd(sgx_param.cmd), version, -1, sgx_param.fid);
...
//	UINT8 *mac_version_buf;
	...
	UINT32 version;
	//UINT32 mac[MAC_SIZE / 4];
	//UINT8 tpmac[MAC_SIZE]; //해당 지역 죽었을까봐 넘긴다.
	UINT8 mac[MAC_SIZE];
...
//	mac[0]=100;
//	for(i=0; i<MAC_SIZE; i+=4)
///		uart_printf("0x%x 0x%x 0x%x 0x%x", (char)mac[i], (char)mac[i+1], (char)mac[i+2], (char)mac[i+3]);
//	uart_printf("vs %d", version);
//	uart_printf("hb %d", read_dram_8(HMAC_BUFF));
//	uart_printf("%d", read_dram_8(&data))
...
//		uart_print("maciszero");
	...
	//if(1)
	if(DS_auth_write(version, sgx_param, (sector_count-1)*512, data, mac, (UINT8*)DS_get_filekey(sgx_param.fid)))
	...
	//	uart_printf("[DS_write_wr] authenticaiton fails..");
		//이미 DRAM WRITEBUFFER에는 데이터가 들어온 상황임.
		//실제로 쓰지 않으면서 컨트롤러에게는 쓴척 해야함.
		//에러 위험성?
		uart_print("Write verification fails");
		if(data_end % SECTORS_PER_PAGE == 0)
		{	
				flash_finish();
				while (g_ftl_write_buf_id == GETREG(SATA_WBUF_PTR));	// bm_write_limit should not outpace SATA_WBUF_PTR
...
static UINT32 DS_extract_mac_version(const UINT32 offset, UINT8 *mac, UINT32 *version, UINT32 num_page)
{
	...
	for(i=0; i<MAC_SIZE; i++)
	{
		...
		//uart_printf("mac 0x%x",read_dram_8(&mac_version_buf[i]));
...
static UINT32 	DS_extract_mac_version(const UINT32 offset, UINT8 *mac, UINT32 *version, UINT32 num_page)
{
	...
	//little endian이므로 수정 가능성 있으나, 지금은 일단 진행:
	//if(offset>4000)
	//uart_printf("%x", mac_version_buf);
	//for(i=0; i<KEY_SIZE; i+=4)
	
	if(offset>4000)
	{	
		//uart_printf("%x %x %x", HMAC_BUFF, (UINT32)mac_version_buf,DRAM_BASE);
		mem_copy(HMAC_BUFF, (UINT32)mac_version_buf, DRAM_ECC_UNIT);
	...
	for(i=0; i<MAC_SIZE; i+=4)
	{
		//mac[i] = read_dram_8(&mac_version_buf[i]);
		if(offset>4000)
			rd=read_dram_32(HMAC_BUFF+i);
		else
			rd=read_dram_32(&mac_version_buf[i]);
...
/*static*/ UINT8 DS_auth_write(UINT32 version, SGX_PARAM sgx_param, UINT32 data_size, UINT8* data_buf, UINT8* mac_host, UINT8* key)
{
...
//	uart_printf("DS_aw: fid(%d), v(%d), size(%d) offset(%d), key[0](%x)", sgx_param.fid, version, data_size,  sgx_param.offset, key[0]);
...
//	uart_printf("mac %d", mac_host[0]);
	...
	*/
	...
	//offset, size
	//uart_printf("offset(%d) size(%d)", sgx_param.offset,tp_ds );
	...
	//uart_printf(",offset(%d)", tp_buf[4+1+4])
	...
	mem_copy(buf+data_size,tp_buf, 4+4+4+4+4);	//반드시 4byte
...
static void DS_rdafwr_make_mac(UINT32 version, UINT32 return_msg, UINT8 *mac, UINT32 file_size, UINT8* key)
{
	...
	//for(i=0; i<32; i+=4)
	i=0;
	//uart_printf("rdafwr v(%d), r(%d) k(%x)", version, return_msg, key[0]);
	//uart_printf("m(%d) m(%d) k(%d)", mac[0], mac[1], key[0]);	
	key[0]=key[0];
	mac[0]=mac[0];
	mac[1]=mac[1];
}
static UINT8 DS_auth_open(UINT32 version, UINT8 *name, UINT8 *mac_host, UINT8* key)
{
	...
	buf_[NAME_LEN] = DS_OPEN_WR;

	for(i=NAME_LEN+1; i<NAME_LEN+1+4; i++)
	{
		buf_[i] = version & 0xff;
		version = version >> 8;
	}

	mem_copy(buf, buf_, NAME_LEN+1+4+3); //4바이트 단위
	return HMAC_authentication(key, buf, size, mac_host);
}
...
void sata_test()
{
	...
	DS_rdafwr_make_mac(version, return_msg, mac_host, -1, key);

//	DS_auth_create(name, key, mac_host);
}
static UINT8 DS_auth_create(UINT8* name, UINT8* key, UINT8* mac_host)
{
	UINT32 buf = HMAC_BUFF+DRAM_ECC_UNIT;	//64 는 input block size
	UINT8 buf_[NAME_LEN+1+KEY_SIZE+3];
	UINT32 size=NAME_LEN+1+KEY_SIZE;
	int i;

	for(i=0; i<NAME_LEN; i++)
		buf_[i]=name[i];

	buf_[NAME_LEN] = DS_CREATE_WR;
	
	for(i=0; i<KEY_SIZE; i++)
		buf_[i+NAME_LEN+1]=key[i];

	mem_copy(buf, buf_, NAME_LEN+1+KEY_SIZE+3); //4바이트 단위

	//uart_printf("size(%d) name(%s)", size, name);
	//uart_printf("key(%d)mac(%d)", key[0], mac[0]);
	return HMAC_authentication(key, buf, size, mac_host);
}
...
static UINT8 HMAC_authentication(UINT8 *key, UINT8* buf, UINT32 size, UINT8* mac_host)
{
	...
	//uart_printf("mac %d %d", mac_device[0], mac_device[1]);
	//uart_printf("key(%d), mac(%d)", key[0], mac_device[0]);
	for(i=0; i<MAC_SIZE; i++)
	{
		//인증 실패
